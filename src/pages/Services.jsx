    ...
    <div className="min-h-screen bg-white dark:bg-[#05070a] text-slate-900 dark:text-white pb-32 transition-colors duration-500">
      ...
      <section className="relative h-[70vh] flex items-center justify-center overflow-hidden mb-20">
        ...
        <div className="relative z-20 text-center px-6">
          ...
          >
            ELITE <span className="text-[#6db70e]">SERVICES</span>
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ delay: 0.4 }}
            className="text-slate-500 dark:text-slate-400 text-lg md:text-xl font-serif italic max-w-2xl mx-auto"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {mainServices.map((service, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="group relative bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-[2.5rem] p-10 backdrop-blur-md hover:border-[#6db70e]/50 transition-all duration-500"
            >
              <div className="flex justify-between items-start mb-12">
                <div className="w-20 h-20 rounded-3xl bg-[#6db70e]/10 flex items-center justify-center text-[#6db70e] group-hover:scale-110 group-hover:bg-[#6db70e] group-hover:text-black transition-all duration-500">
                  {service.icon}
                </div>
                <span className="text-[10px] font-black tracking-[0.4em] text-[#6db70e] border border-[#6db70e]/30 px-4 py-2 rounded-full">
                  {service.tag}
                </span>
              </div>

              <h3 className="text-3xl font-black tracking-tight mb-4 text-slate-900 dark:text-white group-hover:text-[#6db70e] transition-colors">
                {service.title}
              </h3>
              <p className="text-slate-600 dark:text-slate-400 text-lg mb-10 leading-relaxed max-w-sm">
                {service.description}
              </p>
              ...
              <div className="space-y-4 mb-12">
                {service.features.map((feature, i) => (
                  <div key={i} className="flex items-center gap-3 text-slate-700 dark:text-slate-300">
                    <div className="w-1.5 h-1.5 rounded-full bg-[#6db70e]" />
              ...
              <div className="pt-8 border-t border-slate-200 dark:border-white/5 flex justify-between items-center">
                <span className="text-[#6db70e] text-sm font-black uppercase tracking-widest">{service.price}</span>
                <Link to="/contact" className="w-12 h-12 rounded-full border border-slate-200 dark:border-white/10 flex items-center justify-center text-slate-900 dark:text-white hover:bg-[#6db70e] hover:text-black hover:border-[#6db70e] transition-all">
      ...
      <section className="px-6 md:px-24">
        <div className="bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-[3rem] p-12 backdrop-blur-md">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12">
            {additionalServices.map((service, index) => (
              ...
              >
                <div className="text-[#6db70e] mb-6 flex justify-center text-3xl group-hover:scale-125 transition-transform duration-500">
                  {service.icon}
                </div>
                <h4 className="text-xs font-black uppercase tracking-[0.3em] mb-2 text-slate-900 dark:text-white">{service.title}</h4>
                <p className="text-slate-500 text-[10px] uppercase tracking-widest">{service.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="mt-40 text-center px-6">
        <motion.h2 
          ...
          className="text-4xl md:text-6xl font-black tracking-tighter mb-12 text-slate-900 dark:text-white"
        >
          READY TO <span className="text-[#6db70e]">EXPERIENCE</span> ELITE?
        </motion.h2>
        <div className="flex flex-col sm:flex-row gap-6 justify-center items-center">
          <Link to="/allmeals">
            <button className="px-12 py-5 bg-[#6db70e] text-black font-black rounded-full text-xs uppercase tracking-[0.3em] hover:bg-slate-900 hover:text-white transition-all shadow-[0_20px_40px_rgba(109,183,14,0.3)]">
              Initiate Order
            </button>
          </Link>
          <Link to="/contact">
            <button className="px-12 py-5 border border-slate-200 dark:border-white/10 text-slate-900 dark:text-white font-black rounded-full text-xs uppercase tracking-[0.3em] hover:bg-white hover:text-black hover:border-white transition-all">
              Consult Concierge
            </button>
          </Link>
        </div>
